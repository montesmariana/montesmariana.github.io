...
function colorStars(confVar) {
    d3.selectAll("#"+confVar).selectAll("label")
        .style("color", function (d) {
            var here = d3.select(this.parentNode.parentNode.parentNode.parentNode.parentNode).attr("token_id");
            if (!hasSense(here, type) ||
                d3.keys(text[type][here]).indexOf(confVar) === -1 ||
                text[type][here][confVar] < d) {
                return ("#bdbdbd");
            } else {
                return ("#ffa500");
            }
