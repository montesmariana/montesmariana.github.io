...
var colnames, nominals, numerals;
var colorvar, shapevar, sizevar;
var colorselection, shapeselection;

function initVars(data, level, type) {
  colnames = classify_colnames(data);
...
function execute(data, dist) {
    const modx = d3.min(getValues(data, 'model.x'));
    console.log(modx);
    console.log(dist);
}